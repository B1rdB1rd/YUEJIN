          ...
          <el-row class="level1" v-for="level1 in scope.row.children" :key="level1.id">
            ...
            <el-col :span="20">
              ...
              <el-row class="level2" v-for="level2 in level1.children" :key="level2.id">
                <el-col :span="4">
                  <el-tag @close="delRight(scope.row, level2.id)" closable type="success">{{level2.authName}}</el-tag>
                  <i class="el-icon-arrow-right"></i>
                </el-col>
                <el-col :span="20">
                  <!-- 三级 -->
                  <el-tag @close="delRight(scope.row, level3.id)" class="level3" closable v-for="level3 in level2.children" :key="level3.id" type="danger">{{level3.authName}}</el-tag>
                </el-col>
              </el-row>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    showEditDialog(row) {
      ...
      // 如果this.editForm = row  this.editForm直接指向了row
      // this.editForm发生了改变， row跟着改变了
